...
const RateInfoModal = ({open, setOpen,  selectFilm,isUser=true }) => {
  ...
  const [addToFavorite,{isLoading:isLiking,isSuccess,error:errorFilm}]=userApi.useLikeFilmMutation()
  const {data:filmData,isLoading:isGettingFilm,isSuccess:successFilm,refetch}=useGetFilmByIdQuery(selectFilm?.imdb_id)
  const [removeFromFavorite,{isLoading:isDeleting,isSuccess:isDeleted}]=userApi.useDislikeFilmMutation()
  const [updateFilm,{isLoading:isUpdating,error,isSuccess:isUpdated}]=filmApi.useUpdateFilmMutation()
  ...
  useEffect(()=>{
    try{
      // console.log(selectFilm?.imdb_id,filmData.film.isFavorite);
    ...
    <ModalContainer open={open} setOpen={setOpen} position={'flex-end'} padding={0} type={"fade"}>
      ...
      <View style={styles.container}>
        <View style={{...styles.block,flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
          <Text style={{ ...styles.title,flex:1 }} numberOfLines={1}>{chosenFilm?.title}</Text>
