...
const MenuListItem = ({item, index, vItems}) => {
  ...
  const rStyle = useAnimatedStyle(() => {
    const isVisible = Boolean(
      vItems.value
        ...
        .find(viewItem => viewItem?.item.id === item.id),
    );
    return {
  ...
  return (
    <Animated.View key={index} style={[{width: 150, height: 300}, rStyle]}>
      <Pressable
        onPress={e => {
          e.stopPropagation();
        ...
        <Image
          source={{uri: IMG_URI + item.poster_path}}
          style={style.carouselImage}
          resizeMode="contain"
        />
        <Text style={style.carouselText} numberOfLines={1}>
          {item.title}
        </Text>
        <Text style={style.greyText} numberOfLines={1}>
