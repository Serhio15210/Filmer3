...
export const getFavoriteFilms = async () => {
  ...
  const token = await loadToken();
  return axios
    .get(`${BASE_URL}/auth/getFavorites`, {
      headers: {
        Authorization: token,
        language: locale,
      },
...
export const addFavoriteFilm = async film => {
  ...
  const locale = await AsyncStorage.getItem('locale');
  return axios
    .post(`${BASE_URL}/auth/likeFilm`, film, {
      headers: {
        Authorization: token,
        language: locale,
      },
