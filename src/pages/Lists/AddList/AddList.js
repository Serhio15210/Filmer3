...
import { BLACK, MAIN_GREY_FADE, MAIN_GREY_SKELETON, MAIN_RED, MAIN_SUCCESS } from "../../../constants/colors";
...
const AddList = (props) => {
  ...
  const [saveList, { isLoading, error, isSuccess,data }] = userApi.useCreateListMutation();
  const [name,setName]=useState('')
  useEffect(() => {
    if (isSuccess ){
        navigation.reset({
          index:0,
          routes:[{name:'HomeRoot'},{name:'ListOverview',params:{title:name,id:data?._id}}]
        })
    }
    if (error){
      Toast.show({
  ...
  useEffect(() => {
    const keyboardDidShowListener = Keyboard.addListener(
      "keyboardDidShow",
      (e) => {
        marginTop.value = withTiming(normalize(50), { duration: 300 });
      },
    );
  ...
  return (
    <ImageBackground source={addListBg} style={styles.container} resizeMode={"cover"}>
      <Animated.View style={[{
        ...
        <Formik
          ...
          onSubmit={async values => {
            await saveList({ name:values.name, description:values.description, mode });
             setName(values.name)
          }}
        ...
        >
          {({ handleChange, handleBlur, handleSubmit, values }) => (
            <View style={styles.form}>
              ...
              >
                {isLoading?<ActivityIndicator size="large" color={'white'} />:<Text style={styles.text}>{i18n.t("save")}</Text>}
              </AwesomeButton>
              <View
