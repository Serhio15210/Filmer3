...
const FindFilmToList = () => {
  ...
  const [search, { isLoading, isSuccess, data, error }] = searchMovieApi.useGetMovieByQueryMutation();
  const {
    tempFilms,
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TextInput style={{ flex: 1, paddingVertical: 0 }} placeholder={"Add a film..."} value={query}
      ...
      {!query ?
        <>
          <Text style={{ paddingHorizontal: 20, paddingVertical: 10, borderWidth: 1, borderColor: MAIN_GREY_SKELETON }}>Recent
            searches</Text>
          <FlatList data={recentSearches}
                    maxToRenderPerBatch={3}
                    showsVerticalScrollIndicator={false}
                    keyExtractor={(item, index) => "key" + index}
                    contentContainerStyle={{padding:20}}
                    renderItem={({ item, index }) => {
                      return <Pressable style={{ width: "100%" }} onPress={()=>setQuery(item)}>
                        <Text>{item}</Text>
                      </Pressable>;
                    }
                    } />
        </> :
        <FlatList ref={scrollRef} data={searchData?.filter(item=>item?.id&&!dublicateFilms?.includes(item.id+''))}
                  onScroll={(e)=>{
                  ...
                  renderItem={({ item, index }) => {
                    return   <FindFilmCard item={item} index={index} onPress={  () => {
      ...
      {scrollPosition>100&&<Pressable style={styles.up} onPress={() => {
