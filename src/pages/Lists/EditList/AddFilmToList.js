...
const AddFilmToList = ({ route }) => {
  ...
  const {data:filmData,isLoading:isGettingFilm,isSuccess:successFilm,refetch}=useGetFilmByIdQuery(id)
  const [create, {  isSuccess,data:createData, error,isLoading:isCreating  }] = listApi.useCreateFilmMutation();
  const {
    tempFilms,
  ...
  const director = item?.credits?.cast?.filter(c => c?.known_for_department === "Directing")[0];
  useEffect(() => {

    if (isSuccess){
      const pos = parseInt(position) > tempFilms?.length + 1 ? tempFilms?.length + 1 : parseInt(position) - 1;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.info}>
          ...
          <View>
            <View style={{ ...styles.row, alignItems: "flex-end", flexWrap: "wrap", width: "100%" }}>
              <Text numberOfLines={4} style={{ color: "white", fontSize: normalize(18) }}>{item?.title}</Text>
              <Text style={styles.text}>{item?.release_date?.split("-")[0]}</Text>
            ...
            <View style={styles.row}>
              <AntDesign name="star" size={20}
                         color={"#FFD700"} />
              <Text style={styles.text}>{Math.round(item?.vote_average * 10) / 10}</Text>
            </View>
          </View>
        ...
        <View style={styles.positionRow}>
          ...
          <TextInput style={{ ...styles.input, width: inputWidth }} value={position} onChangeText={setPosition} />
        </View>
        <AwesomeButton
          progress
          onPress={ (next) => {
...
const styles = StyleSheet.create({
  ...
  info: {
    flexDirection: "row", gap: 10, borderBottomWidth: 1, borderColor: MAIN_GREY_SKELETON, paddingBottom: 10,
  },
  img: {
    width: normalize(70), height: normalize(100),
  },
  text: {
  ...
  input: {
    padding: 0, textAlign: "center", borderBottomWidth: 1, borderColor: MAIN_GREY_SKELETON,
  },
});
