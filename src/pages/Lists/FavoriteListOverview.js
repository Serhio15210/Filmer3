...
import { useGetUserFavoriteListsQuery, userApi } from "../../services/UserService";
...
const FavoriteListOverview = ({route}) => {
  ...
  const [removeFromFavorite,{isLoading:isDeleting,error:errorDeleting,isSuccess:isDeleted}]=userApi.useDislikeFilmMutation()
  const [addToFavorite,{isLoading:isLiking,isSuccess}]=userApi.useLikeFilmMutation()
  ...
  return (
      <View style={styles.container}>
        ...
        {isAddFilm &&
          <FindFilmModal setOpen={setIsAddFilm} open={isAddFilm} isFavorite={true} listData={listData?.favoriteFilms}
        ...
        <View style={{paddingHorizontal: normalize(15), flex: 1}}>
          ...
          {listData?.favoriteFilms?.length === 0 ?
            ...
            isLoading?<Loading/>:
              <FlatList data={listData?.favoriteFilms}
